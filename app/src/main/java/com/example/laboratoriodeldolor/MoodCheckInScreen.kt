...
import com.example.laboratoriodeldolor.ui.accessibility.accessibleHeading
import com.example.laboratoriodeldolor.ui.accessibility.AccessibilityUtils
import com.example.laboratoriodeldolor.ui.accessibility.getMoodDescription
    ...
    com.example.laboratoriodeldolor.ui.AppScaffold { _ ->
        AnimatedVisibility(visible = visible, enter = fadeIn(animationSpec = tween(260)), exit = fadeOut(animationSpec = tween(260))) {
            Column(modifier = Modifier.padding(start = 16.dp, end = 16.dp, top = 32.dp, bottom = 16.dp)) {
                Text(
                    text = stringResource(id = R.string.checkin_mood_title), 
                    style = MaterialTheme.typography.headlineSmall, 
                    modifier = Modifier
                        .testTag("screen_title")
                        .accessibleHeading(AccessibilityUtils.ContentDescriptions.MOOD_SELECTOR)
                )
            ...
            Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.Center) {
                ...
                options.forEachIndexed { idx, e ->
                    ...
                    val desc = when (idx) {
                        0 -> getMoodDescription(e, 1)
                        1 -> getMoodDescription(e, 2)
                        2 -> getMoodDescription(e, 3)
                        3 -> getMoodDescription(e, 4)
                        else -> getMoodDescription(e, 5)
                    }
            ...
            OutlinedTextField(
                ...
                modifier = Modifier
                    ...
                    .height(200.dp)
                    .semantics {
                        contentDescription = AccessibilityUtils.ContentDescriptions.MOOD_NOTE_INPUT
                    }, 
                placeholder = { Text(text = stringResource(id = R.string.checkin_mood_hint)) },
                label = { Text("Optional mood note") }
            )
